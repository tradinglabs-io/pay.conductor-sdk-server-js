...
#' @docType class
#' @title Cliente
#' @description Cliente Class
#' @format An \code{R6Class} generator object
#' @field address  \link{EndereODoCliente} [optional]
#' @field documentNumber CPF ou CNPJ do cliente sem formatação character
#' @field documentType  character
#' @field email Email do cliente character
#' @field name Nome completo do cliente character
#' @field phoneNumber Número de telefone do cliente no formato +55 DD 9XXXXXXXX character [optional]
...
Cliente <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new Cliente class.
    #'
    ...
    #'
    #' @return Cliente as a base R list.
    #' @examples
    #' # convert array of Cliente (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Deserialize JSON string into an instance of Cliente
    #'
    #' @param input_json the JSON input
    #' @return the instance of Cliente
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`address`)) {
        `address_object` <- EndereODoCliente$new()
    ...
    fromJSONString = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      self$`address` <- EndereODoCliente$new()$fromJSON(jsonlite::toJSON(this_object$`address`, auto_unbox = TRUE, digits = NA))
      self$`documentNumber` <- this_object$`documentNumber`
      if (!is.null(this_object$`documentType`) && !(this_object$`documentType` %in% c("Cpf", "Cnpj"))) {
        stop(paste("Error! \"", this_object$`documentType`, "\" cannot be assigned to `documentType`. Must be \"Cpf\", \"Cnpj\".", sep = ""))
      }
    ...
    #' @description
    #' Validate JSON input with respect to Cliente and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`documentNumber`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Cliente: the required field `documentNumber` is missing."))
      }
      ...
      if (!is.null(input_json$`documentType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Cliente: the required field `documentType` is missing."))
      }
      ...
      if (!is.null(input_json$`email`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Cliente: the required field `email` is missing."))
      }
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Cliente: the required field `name` is missing."))
      }
    },
...
# Cliente$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
