#' Create a new DadosCompletosDoCartO
#'
#' @description
#' DadosCompletosDoCartO Class
#'
#' @docType class
#' @title DadosCompletosDoCartO
#' @description DadosCompletosDoCartO Class
#' @format An \code{R6Class} generator object
#' @field cvv Código de segurança do cartão (CVV) character
#' @field expiration  \link{DadosCompletosDoCartOExpiration}
#' @field holderName Nome do titular do cartão character
#' @field number Número do cartão de crédito character
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
DadosCompletosDoCartO <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new DadosCompletosDoCartO class.
    #'
    ...
    #'
    #' @return DadosCompletosDoCartO as a base R list.
    #' @examples
    #' # convert array of DadosCompletosDoCartO (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert DadosCompletosDoCartO to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      DadosCompletosDoCartOObject <- list()
      if (!is.null(self$`cvv`)) {
        DadosCompletosDoCartOObject[["cvv"]] <-
          self$`cvv`
      }
      if (!is.null(self$`expiration`)) {
        DadosCompletosDoCartOObject[["expiration"]] <-
          self$extractSimpleType(self$`expiration`)
      }
      if (!is.null(self$`holderName`)) {
        DadosCompletosDoCartOObject[["holderName"]] <-
          self$`holderName`
      }
      if (!is.null(self$`number`)) {
        DadosCompletosDoCartOObject[["number"]] <-
          self$`number`
      }
      return(DadosCompletosDoCartOObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of DadosCompletosDoCartO
    #'
    #' @param input_json the JSON input
    #' @return the instance of DadosCompletosDoCartO
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`expiration`)) {
        `expiration_object` <- DadosCompletosDoCartOExpiration$new()
    ...
    #' @return DadosCompletosDoCartO in JSON format
    ...
    fromJSONString = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      self$`cvv` <- this_object$`cvv`
      self$`expiration` <- DadosCompletosDoCartOExpiration$new()$fromJSON(jsonlite::toJSON(this_object$`expiration`, auto_unbox = TRUE, digits = NA))
      self$`holderName` <- this_object$`holderName`
      self$`number` <- this_object$`number`
      self
    },
    ...
    #' @description
    #' Validate JSON input with respect to DadosCompletosDoCartO and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`cvv`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for DadosCompletosDoCartO: the required field `cvv` is missing."))
      }
      # check the required field `expiration`
      if (!is.null(input_json$`expiration`)) {
        stopifnot(R6::is.R6(input_json$`expiration`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for DadosCompletosDoCartO: the required field `expiration` is missing."))
      }
      ...
      if (!is.null(input_json$`holderName`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for DadosCompletosDoCartO: the required field `holderName` is missing."))
      }
      ...
      if (!is.null(input_json$`number`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for DadosCompletosDoCartO: the required field `number` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of DadosCompletosDoCartO
    toString = function() {
      self$toJSONString()
    },
...
# DadosCompletosDoCartO$unlock()
#
## Below is an example to define the print function
# DadosCompletosDoCartO$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# DadosCompletosDoCartO$lock()
