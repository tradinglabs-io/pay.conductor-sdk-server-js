...
#' @field expirationInDays  \link{BoletoExpirationInDays} [optional]
...
Boleto <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return Boleto as a base R list.
    #' @examples
    #' # convert array of Boleto (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert Boleto to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      BoletoObject <- list()
      if (!is.null(self$`paymentMethod`)) {
        BoletoObject[["paymentMethod"]] <-
          self$`paymentMethod`
      }
      if (!is.null(self$`expirationInDays`)) {
        BoletoObject[["expirationInDays"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of Boleto
    #'
    #' @param input_json the JSON input
    #' @return the instance of Boleto
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`expirationInDays`)) {
        `expirationindays_object` <- BoletoExpirationInDays$new()
    ...
    fromJSONString = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      self$`paymentMethod` <- this_object$`paymentMethod`
      self$`expirationInDays` <- BoletoExpirationInDays$new()$fromJSON(jsonlite::toJSON(this_object$`expirationInDays`, auto_unbox = TRUE, digits = NA))
      self
    },

    #' @description
    #' Validate JSON input with respect to Boleto and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`paymentMethod`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Boleto: the required field `paymentMethod` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of Boleto
    toString = function() {
      self$toJSONString()
    },
...
# Boleto$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
