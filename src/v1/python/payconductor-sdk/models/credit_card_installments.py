...
CREDITCARDINSTALLMENTS_ANY_OF_SCHEMAS = ["int"]

class CreditCardInstallments(BaseModel):
    """
    ...
    # data type: int
    anyof_schema_2_validator: Optional[Annotated[int, Field(le=12, strict=True, ge=1)]] = Field(default=1, description="Number of installments")
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CreditCardInstallments.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in CreditCardInstallments with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CreditCardInstallments with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return instance
