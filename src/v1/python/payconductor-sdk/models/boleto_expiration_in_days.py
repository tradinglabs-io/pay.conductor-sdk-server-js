...
BOLETOEXPIRATIONINDAYS_ANY_OF_SCHEMAS = ["int"]

class BoletoExpirationInDays(BaseModel):
    """
    ...
    # data type: int
    anyof_schema_2_validator: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=7, description="Dias até o vencimento do boleto")
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = BoletoExpirationInDays.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in BoletoExpirationInDays with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BoletoExpirationInDays with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return instance
