...
PIXEXPIRATIONINSECONDS_ANY_OF_SCHEMAS = ["int"]

class PixExpirationInSeconds(BaseModel):
    """
    ...
    # data type: int
    anyof_schema_2_validator: Optional[Annotated[int, Field(strict=True, ge=3600)]] = Field(default=3600, description="Tempo de expiração do PIX em segundos")
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PixExpirationInSeconds.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PixExpirationInSeconds with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PixExpirationInSeconds with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return instance
