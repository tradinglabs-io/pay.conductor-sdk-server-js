...
-- model import
local payconductor_sdk_order_confirm_response = require "payconductor_sdk.model.order_confirm_response"
...
local payconductor_sdk_order_payment_request = require "payconductor_sdk.model.order_payment_request"
...
function order_api:order_confirm(id, order_payment_request)
	...
	req:set_body(dkjson.encode(order_payment_request))
	...
	if http_status:sub(1,1) == "2" then
		...
		return payconductor_sdk_order_confirm_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function order_api:order_create(order_create_request)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	req:set_body(dkjson.encode(order_create_request))
	...
	if http_status:sub(1,1) == "2" then
		...
		return payconductor_sdk_order_create_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function order_api:order_list(page, page_size, end_date, start_date, id, status)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function order_api:order_read(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function order_api:order_refund(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
