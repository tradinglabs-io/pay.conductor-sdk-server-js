...
local payconductor_sdk_withdraw_create_request = require "payconductor_sdk.model.withdraw_create_request"
...
function withdraw_api:withdraw_create(withdraw_create_request)
	...
	req:set_body(dkjson.encode(withdraw_create_request))
	...
	if http_status:sub(1,1) == "2" then
		...
		return payconductor_sdk_withdraw_create_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function withdraw_api:withdraw_list(page, page_size, end_date, start_date, id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function withdraw_api:withdraw_read(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
