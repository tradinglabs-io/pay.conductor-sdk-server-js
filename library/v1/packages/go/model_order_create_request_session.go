/*
PayConductor API

PayConductor API Documentation.  This documentation covers all available features in the PayConductor RESTful API, including authentication, resource management, and usage examples.  <br />  <br />  ----  <br />   # Authentication  PayConductor API uses HTTP Basic authentication to validate requests. You need to provide your credentials (Client ID and Client Secret) in the `client:secret` format encoded in Base64.  <br />  ## Getting Credentials  1. Access the PayConductor admin panel 2. Navigate to **Settings > API Keys** 3. Generate a new credentials pair (Client ID and Client Secret) 4. Store the Client Secret in a secure location - it will not be displayed again  <br />  ## Authentication Format  Credentials must be sent in the `Authorization` header using the Basic scheme:  ``` Authorization: Basic base64(client_id:client_secret) ```  <br />  ## Node.js Example  ```javascript const clientId = 'your_client_id'; const clientSecret = 'your_client_secret';  // Encode credentials in Base64 const credentials = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');  const response = await fetch('https://api.payconductor.com/api/v1/orders', {   method: 'GET',   headers: {     'Authorization': `Basic ${credentials}`,     'Content-Type': 'application/json'   } });  const data = await response.json(); console.log(data); ```  <br />  ## Authentication Errors  | Code | Description | |------|-------------| | `401` | Invalid or missing credentials | | `403` | Valid credentials but no permission for the resource | | `429` | Too many requests (rate limit exceeded) |  ### Error Response Example  ```json {   \"error\": {     \"code\": \"UNAUTHORIZED\",     \"message\": \"Invalid credentials\",     \"details\": \"The provided client ID or secret is incorrect\"   } } ```

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package payconductor_sdk

import (
	"encoding/json"
)

// checks if the OrderCreateRequestSession type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &OrderCreateRequestSession{}

// OrderCreateRequestSession If externalSessionId or sessionId is provided and an existing session exists, it will be updated with the new data. Otherwise, a new session will be created.
type OrderCreateRequestSession struct {
	// Time in minutes for checkout session expiration
	ExpiresInMinutes *float32 `json:"expiresInMinutes,omitempty"`
	// Checkout URL for redirection
	CheckoutUrl *string `json:"checkoutUrl,omitempty"`
	// ID generated by our service to identify the checkout session
	SessionId *string `json:"sessionId,omitempty"`
	// Checkout session ID generated by the client side. The ID must be unique for each cart session
	ExternalSessionId *string `json:"externalSessionId,omitempty"`
}

// NewOrderCreateRequestSession instantiates a new OrderCreateRequestSession object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewOrderCreateRequestSession() *OrderCreateRequestSession {
	this := OrderCreateRequestSession{}
	var expiresInMinutes float32 = 30
	this.ExpiresInMinutes = &expiresInMinutes
	return &this
}

// NewOrderCreateRequestSessionWithDefaults instantiates a new OrderCreateRequestSession object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewOrderCreateRequestSessionWithDefaults() *OrderCreateRequestSession {
	this := OrderCreateRequestSession{}
	var expiresInMinutes float32 = 30
	this.ExpiresInMinutes = &expiresInMinutes
	return &this
}

// GetExpiresInMinutes returns the ExpiresInMinutes field value if set, zero value otherwise.
func (o *OrderCreateRequestSession) GetExpiresInMinutes() float32 {
	if o == nil || IsNil(o.ExpiresInMinutes) {
		var ret float32
		return ret
	}
	return *o.ExpiresInMinutes
}

// GetExpiresInMinutesOk returns a tuple with the ExpiresInMinutes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderCreateRequestSession) GetExpiresInMinutesOk() (*float32, bool) {
	if o == nil || IsNil(o.ExpiresInMinutes) {
		return nil, false
	}
	return o.ExpiresInMinutes, true
}

// HasExpiresInMinutes returns a boolean if a field has been set.
func (o *OrderCreateRequestSession) HasExpiresInMinutes() bool {
	if o != nil && !IsNil(o.ExpiresInMinutes) {
		return true
	}

	return false
}

// SetExpiresInMinutes gets a reference to the given float32 and assigns it to the ExpiresInMinutes field.
func (o *OrderCreateRequestSession) SetExpiresInMinutes(v float32) {
	o.ExpiresInMinutes = &v
}

// GetCheckoutUrl returns the CheckoutUrl field value if set, zero value otherwise.
func (o *OrderCreateRequestSession) GetCheckoutUrl() string {
	if o == nil || IsNil(o.CheckoutUrl) {
		var ret string
		return ret
	}
	return *o.CheckoutUrl
}

// GetCheckoutUrlOk returns a tuple with the CheckoutUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderCreateRequestSession) GetCheckoutUrlOk() (*string, bool) {
	if o == nil || IsNil(o.CheckoutUrl) {
		return nil, false
	}
	return o.CheckoutUrl, true
}

// HasCheckoutUrl returns a boolean if a field has been set.
func (o *OrderCreateRequestSession) HasCheckoutUrl() bool {
	if o != nil && !IsNil(o.CheckoutUrl) {
		return true
	}

	return false
}

// SetCheckoutUrl gets a reference to the given string and assigns it to the CheckoutUrl field.
func (o *OrderCreateRequestSession) SetCheckoutUrl(v string) {
	o.CheckoutUrl = &v
}

// GetSessionId returns the SessionId field value if set, zero value otherwise.
func (o *OrderCreateRequestSession) GetSessionId() string {
	if o == nil || IsNil(o.SessionId) {
		var ret string
		return ret
	}
	return *o.SessionId
}

// GetSessionIdOk returns a tuple with the SessionId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderCreateRequestSession) GetSessionIdOk() (*string, bool) {
	if o == nil || IsNil(o.SessionId) {
		return nil, false
	}
	return o.SessionId, true
}

// HasSessionId returns a boolean if a field has been set.
func (o *OrderCreateRequestSession) HasSessionId() bool {
	if o != nil && !IsNil(o.SessionId) {
		return true
	}

	return false
}

// SetSessionId gets a reference to the given string and assigns it to the SessionId field.
func (o *OrderCreateRequestSession) SetSessionId(v string) {
	o.SessionId = &v
}

// GetExternalSessionId returns the ExternalSessionId field value if set, zero value otherwise.
func (o *OrderCreateRequestSession) GetExternalSessionId() string {
	if o == nil || IsNil(o.ExternalSessionId) {
		var ret string
		return ret
	}
	return *o.ExternalSessionId
}

// GetExternalSessionIdOk returns a tuple with the ExternalSessionId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderCreateRequestSession) GetExternalSessionIdOk() (*string, bool) {
	if o == nil || IsNil(o.ExternalSessionId) {
		return nil, false
	}
	return o.ExternalSessionId, true
}

// HasExternalSessionId returns a boolean if a field has been set.
func (o *OrderCreateRequestSession) HasExternalSessionId() bool {
	if o != nil && !IsNil(o.ExternalSessionId) {
		return true
	}

	return false
}

// SetExternalSessionId gets a reference to the given string and assigns it to the ExternalSessionId field.
func (o *OrderCreateRequestSession) SetExternalSessionId(v string) {
	o.ExternalSessionId = &v
}

func (o OrderCreateRequestSession) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o OrderCreateRequestSession) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ExpiresInMinutes) {
		toSerialize["expiresInMinutes"] = o.ExpiresInMinutes
	}
	if !IsNil(o.CheckoutUrl) {
		toSerialize["checkoutUrl"] = o.CheckoutUrl
	}
	if !IsNil(o.SessionId) {
		toSerialize["sessionId"] = o.SessionId
	}
	if !IsNil(o.ExternalSessionId) {
		toSerialize["externalSessionId"] = o.ExternalSessionId
	}
	return toSerialize, nil
}

type NullableOrderCreateRequestSession struct {
	value *OrderCreateRequestSession
	isSet bool
}

func (v NullableOrderCreateRequestSession) Get() *OrderCreateRequestSession {
	return v.value
}

func (v *NullableOrderCreateRequestSession) Set(val *OrderCreateRequestSession) {
	v.value = val
	v.isSet = true
}

func (v NullableOrderCreateRequestSession) IsSet() bool {
	return v.isSet
}

func (v *NullableOrderCreateRequestSession) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOrderCreateRequestSession(val *OrderCreateRequestSession) *NullableOrderCreateRequestSession {
	return &NullableOrderCreateRequestSession{value: val, isSet: true}
}

func (v NullableOrderCreateRequestSession) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOrderCreateRequestSession) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


