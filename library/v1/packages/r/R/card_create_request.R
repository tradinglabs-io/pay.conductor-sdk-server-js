#' Create a new CardCreateRequest
#'
#' @description
...
CardCreateRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CardCreateRequest class.
    #'
    ...
    #'
    #' @return CardCreateRequest as a base R list.
    #' @examples
    #' # convert array of CardCreateRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CardCreateRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CardCreateRequestObject <- list()
      if (!is.null(self$`cvv`)) {
        CardCreateRequestObject[["cvv"]] <-
          self$`cvv`
      }
      if (!is.null(self$`expiration`)) {
        CardCreateRequestObject[["expiration"]] <-
          self$extractSimpleType(self$`expiration`)
      }
      if (!is.null(self$`holderName`)) {
        CardCreateRequestObject[["holderName"]] <-
          self$`holderName`
      }
      if (!is.null(self$`number`)) {
        CardCreateRequestObject[["number"]] <-
          self$`number`
      }
      return(CardCreateRequestObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of CardCreateRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of CardCreateRequest
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`expiration`)) {
        `expiration_object` <- CardExpirationData$new()
    ...
    #' @return CardCreateRequest in JSON format
    ...
    fromJSONString = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      self$`cvv` <- this_object$`cvv`
      self$`expiration` <- CardExpirationData$new()$fromJSON(jsonlite::toJSON(this_object$`expiration`, auto_unbox = TRUE, digits = NA))
      self$`holderName` <- this_object$`holderName`
      self$`number` <- this_object$`number`
      self
    },
    ...
    #' @description
    #' Validate JSON input with respect to CardCreateRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`cvv`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CardCreateRequest: the required field `cvv` is missing."))
      }
      # check the required field `expiration`
      if (!is.null(input_json$`expiration`)) {
        stopifnot(R6::is.R6(input_json$`expiration`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CardCreateRequest: the required field `expiration` is missing."))
      }
      ...
      if (!is.null(input_json$`holderName`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CardCreateRequest: the required field `holderName` is missing."))
      }
      ...
      if (!is.null(input_json$`number`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CardCreateRequest: the required field `number` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CardCreateRequest
    toString = function() {
      self$toJSONString()
    },
...
# CardCreateRequest$unlock()
#
## Below is an example to define the print function
# CardCreateRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CardCreateRequest$lock()
