#' Create a new OrderNuPayPaymentRequestNuPay
#'
#' @description
#' Specific data for NuPay payment
#'
#' @docType class
#' @title OrderNuPayPaymentRequestNuPay
#' @description OrderNuPayPaymentRequestNuPay Class
...
#' @export
OrderNuPayPaymentRequestNuPay <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new OrderNuPayPaymentRequestNuPay class.
    #'
    ...
    #'
    #' @return OrderNuPayPaymentRequestNuPay as a base R list.
    #' @examples
    #' # convert array of OrderNuPayPaymentRequestNuPay (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert OrderNuPayPaymentRequestNuPay to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      OrderNuPayPaymentRequestNuPayObject <- list()
      if (!is.null(self$`cancelUrl`)) {
        OrderNuPayPaymentRequestNuPayObject[["cancelUrl"]] <-
          self$`cancelUrl`
      }
      if (!is.null(self$`merchantName`)) {
        OrderNuPayPaymentRequestNuPayObject[["merchantName"]] <-
          self$`merchantName`
      }
      if (!is.null(self$`returnUrl`)) {
        OrderNuPayPaymentRequestNuPayObject[["returnUrl"]] <-
          self$`returnUrl`
      }
      if (!is.null(self$`storeName`)) {
        OrderNuPayPaymentRequestNuPayObject[["storeName"]] <-
          self$`storeName`
      }
      return(OrderNuPayPaymentRequestNuPayObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of OrderNuPayPaymentRequestNuPay
    #'
    #' @param input_json the JSON input
    #' @return the instance of OrderNuPayPaymentRequestNuPay
    ...
    #' @return OrderNuPayPaymentRequestNuPay in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to OrderNuPayPaymentRequestNuPay and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`cancelUrl`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrderNuPayPaymentRequestNuPay: the required field `cancelUrl` is missing."))
      }
      ...
      if (!is.null(input_json$`merchantName`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrderNuPayPaymentRequestNuPay: the required field `merchantName` is missing."))
      }
      ...
      if (!is.null(input_json$`returnUrl`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrderNuPayPaymentRequestNuPay: the required field `returnUrl` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of OrderNuPayPaymentRequestNuPay
    toString = function() {
      self$toJSONString()
    },
...
# OrderNuPayPaymentRequestNuPay$unlock()
#
## Below is an example to define the print function
# OrderNuPayPaymentRequestNuPay$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# OrderNuPayPaymentRequestNuPay$lock()
