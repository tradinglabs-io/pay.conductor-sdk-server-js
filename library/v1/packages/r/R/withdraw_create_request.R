#' Create a new WithdrawCreateRequest
#'
#' @description
#' Withdrawal or transfer request data
#'
#' @docType class
#' @title WithdrawCreateRequest
#' @description WithdrawCreateRequest Class
...
#' @field payoutAccount  \link{WithdrawPayoutAccountData}
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
WithdrawCreateRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new WithdrawCreateRequest class.
    #'
    ...
    #'
    #' @return WithdrawCreateRequest as a base R list.
    #' @examples
    #' # convert array of WithdrawCreateRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert WithdrawCreateRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      WithdrawCreateRequestObject <- list()
      if (!is.null(self$`amount`)) {
        WithdrawCreateRequestObject[["amount"]] <-
          self$`amount`
      }
      if (!is.null(self$`externalId`)) {
        WithdrawCreateRequestObject[["externalId"]] <-
          self$`externalId`
      }
      if (!is.null(self$`merchant`)) {
        WithdrawCreateRequestObject[["merchant"]] <-
          self$extractSimpleType(self$`merchant`)
      }
      if (!is.null(self$`payoutAccount`)) {
        WithdrawCreateRequestObject[["payoutAccount"]] <-
          self$extractSimpleType(self$`payoutAccount`)
      }
      return(WithdrawCreateRequestObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of WithdrawCreateRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of WithdrawCreateRequest
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`payoutAccount`)) {
        `payoutaccount_object` <- WithdrawPayoutAccountData$new()
    ...
    #' @return WithdrawCreateRequest in JSON format
    ...
    fromJSONString = function(input_json) {
      ...
      self$`payoutAccount` <- WithdrawPayoutAccountData$new()$fromJSON(jsonlite::toJSON(this_object$`payoutAccount`, auto_unbox = TRUE, digits = NA))
      self
    },

    #' @description
    #' Validate JSON input with respect to WithdrawCreateRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`amount`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for WithdrawCreateRequest: the required field `amount` is missing."))
      }
      # check the required field `payoutAccount`
      if (!is.null(input_json$`payoutAccount`)) {
        stopifnot(R6::is.R6(input_json$`payoutAccount`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for WithdrawCreateRequest: the required field `payoutAccount` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of WithdrawCreateRequest
    toString = function() {
      self$toJSONString()
    },
...
# WithdrawCreateRequest$unlock()
#
## Below is an example to define the print function
# WithdrawCreateRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# WithdrawCreateRequest$lock()
