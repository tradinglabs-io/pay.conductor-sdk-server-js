#' Create a new CustomerCreateRequest
#'
#' @description
...
#' @description CustomerCreateRequest Class
...
#' @export
CustomerCreateRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CustomerCreateRequest class.
    #'
    ...
    #'
    #' @return CustomerCreateRequest as a base R list.
    #' @examples
    #' # convert array of CustomerCreateRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CustomerCreateRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CustomerCreateRequestObject <- list()
      if (!is.null(self$`address`)) {
        CustomerCreateRequestObject[["address"]] <-
          self$extractSimpleType(self$`address`)
      }
      if (!is.null(self$`documentNumber`)) {
        CustomerCreateRequestObject[["documentNumber"]] <-
          self$`documentNumber`
      }
      if (!is.null(self$`documentType`)) {
        CustomerCreateRequestObject[["documentType"]] <-
          self$extractSimpleType(self$`documentType`)
      }
      if (!is.null(self$`email`)) {
        CustomerCreateRequestObject[["email"]] <-
          self$`email`
      }
      if (!is.null(self$`name`)) {
        CustomerCreateRequestObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`phoneNumber`)) {
        CustomerCreateRequestObject[["phoneNumber"]] <-
          self$`phoneNumber`
      }
      return(CustomerCreateRequestObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of CustomerCreateRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of CustomerCreateRequest
    ...
    #' @return CustomerCreateRequest in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to CustomerCreateRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`documentNumber`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CustomerCreateRequest: the required field `documentNumber` is missing."))
      }
      # check the required field `documentType`
      if (!is.null(input_json$`documentType`)) {
        stopifnot(R6::is.R6(input_json$`documentType`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for CustomerCreateRequest: the required field `documentType` is missing."))
      }
      ...
      if (!is.null(input_json$`email`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CustomerCreateRequest: the required field `email` is missing."))
      }
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CustomerCreateRequest: the required field `name` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CustomerCreateRequest
    toString = function() {
      self$toJSONString()
    },
...
# CustomerCreateRequest$unlock()
#
## Below is an example to define the print function
# CustomerCreateRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CustomerCreateRequest$lock()
