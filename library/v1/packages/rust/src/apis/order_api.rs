...
/// struct for typed errors of method [`order_confirm`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OrderConfirmError {
...
/// struct for typed errors of method [`order_read`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`order_refund`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OrderRefundError {
    UnknownValue(serde_json::Value),
}


/// Confirms a Draft order, setting the payment method and processing the charges associated with the order.
pub async fn order_confirm(configuration: &configuration::Configuration, id: &str, order_payment_request: models::OrderPaymentRequest) -> Result<models::OrderConfirmResponse, Error<OrderConfirmError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_body_order_payment_request = order_payment_request;
    ...
    req_builder = req_builder.json(&p_body_order_payment_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::OrderConfirmResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::OrderConfirmResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<OrderConfirmError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Create a new order for payment using the provided data
pub async fn order_create(configuration: &configuration::Configuration, order_create_request: models::OrderCreateRequest) -> Result<models::OrderCreateResponse, Error<OrderCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_order_create_request = order_create_request;
    ...
    req_builder = req_builder.json(&p_body_order_create_request);
...
pub async fn order_list(configuration: &configuration::Configuration, page: f64, page_size: f64, end_date: Option<String>, start_date: Option<String>, id: Option<&str>, status: Option<&str>) -> Result<(), Error<OrderListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
...
pub async fn order_read(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<OrderReadError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OrderReadError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Initiate a refund for the order specified by ID.
pub async fn order_refund(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<OrderRefundError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OrderRefundError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
