...
/// struct for typed errors of method [`withdraw_create`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WithdrawCreateError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`withdraw_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WithdrawListError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`withdraw_read`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WithdrawReadError {
    UnknownValue(serde_json::Value),
}


/// Create a new withdrawal or transfer to the provided payment account
pub async fn withdraw_create(configuration: &configuration::Configuration, withdraw_create_request: models::WithdrawCreateRequest) -> Result<models::WithdrawCreateResponse, Error<WithdrawCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_withdraw_create_request = withdraw_create_request;
    ...
    req_builder = req_builder.json(&p_body_withdraw_create_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WithdrawCreateResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WithdrawCreateResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<WithdrawCreateError> = serde_json::from_str(&content).ok();
...
pub async fn withdraw_list(configuration: &configuration::Configuration, page: f64, page_size: f64, end_date: Option<String>, start_date: Option<String>, id: Option<&str>) -> Result<(), Error<WithdrawListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<WithdrawListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns the details of a specific withdrawal or transfer using its ID
pub async fn withdraw_read(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<WithdrawReadError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<WithdrawReadError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
