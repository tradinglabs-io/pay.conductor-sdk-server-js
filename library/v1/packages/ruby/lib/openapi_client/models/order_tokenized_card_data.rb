...
module OpenapiClient
  class OrderTokenizedCardData < ApiModelBase
    # First 6 digits of the credit card
    attr_accessor :first_six_card_number
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OrderTokenizedCardData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OrderTokenizedCardData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'first_six_card_number')
        self.first_six_card_number = attributes[:'first_six_card_number']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          first_six_card_number == o.first_six_card_number &&
          token == o.token
    end
    ...
    def hash
      [first_six_card_number, token].hash
    end
